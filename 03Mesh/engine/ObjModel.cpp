...
void ObjModel::loadModelCpu(const char* filename, bool isStacked, bool withoutNormals, bool generateSmoothNormals, bool generateFlatNormals, bool generateSmoothTangents, bool rescale) {
	loadModelCpu(filename, glm::vec3(0.0, 1.0, 0.0), 0.0, glm::vec3(0.0, 0.0, 0.0), 1.0, isStacked, withoutNormals, generateSmoothNormals, generateFlatNormals, generateSmoothTangents, rescale);
}
...
void ObjModel::loadModelCpu(const char* _filename, const glm::vec3& axis, float degree, const glm::vec3& translate, float scale, bool isStacked, bool withoutNormals, bool generateSmoothNormals, bool generateFlatNormals, bool generateSmoothTangents, bool rescale) {
		...
		case '#': {
			...
			switch (buffer[1]) {
				...
				case '\0': {
					...
					glm::vec3 tmp = rot * glm::vec4(tmpx, tmpy, tmpz, 1.0f);
					...
					if (!withoutNormals) {
						...
						glm::mat4 rot = glm::rotate(Camera::IDENTITY, degree * PI_ON_180, axis);
						glm::vec3 tmp = rot * glm::vec4(tmpx, tmpy, tmpz, 0.0f);
